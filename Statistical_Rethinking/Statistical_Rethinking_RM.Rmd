...
# Create ranges of air quality measure (ug/m^3)
pm.air <- c(5, 6, 10, 12, 15, 20, 22, 30, 25, 27)

# Number of trees (100 per hectare)
tree.number <- c(40, 30, 25, 18, 15, 18.5, 16, 5, 8, 10)

# Number of factories in the area
factory.number <- c(15, 17, 25, 20, 30, 28, 32, 40, 18, 28)
...
d1 <- ggplot(d, aes(x = tree.number, y = pm.air)) +
  geom_point(color = col.alpha("green3", 0.8), size = 2) +
  geom_smooth(method = "lm", fullrange = T,
...
d2 <- ggplot(d, aes(x = factory.number, y = pm.air)) +
  geom_point(color = col.alpha("red3", 0.8), size = 2) +
  geom_smooth(method = "lm", fullrange = T,
              color  = "red4", size = 1/2, 
              fill   = "red", alpha = 1/3) +
 ...
 # number of trees vs. number of factories
 m.tf <- lm(tree.number ~ 1 + factory.number, data = d)
 precis(m.tf)
 d3 <- ggplot(d, aes(x = factory.number, y = tree.number)) +
  geom_point(color = col.alpha("purple3", 0.8), size = 2) +
  geom_smooth(method = "lm", fullrange = T,
              color  = "purple4", size = 1/2, 
              fill   = "purple", alpha = 1/3) +
  labs(x = "Number of factories", y = "Particulate matter (ug/m^3)") +
  theme_bw() 
 d3
 ...
 precis(m.atf)
 pairs(~ pm.air + tree.number + factory.number, data = d, col = "blue")
...
sum(diff<0)/1e5
# This is only of heuristic value, especially with only 12 cases. On the other hand, for having only 12 cases, this is more than we might have reasonably hoped for.
