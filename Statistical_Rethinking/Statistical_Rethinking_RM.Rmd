...
## Simulate King Markov’s journey

# Set number of weeks to simulate
num_weeks <- 1e5

# Define an empty history vector
positions <- rep(0,num_weeks)

# Define a starting island position (the biggest island, number 10)
current <- 10
...
# Plot the data (follow ajkurz's codes)
tibble(week   = 1:1e5,
       island = positions) %>%
...
output <- capture.output(m8.1stan <- ulam(
    alist(
        log_gdp ~ dnorm( mu , sigma ) ,
        mu <- a + bR*rugged + bA*cont_africa + bAR*rugged*cont_africa ,
        a ~ dnorm(0,100),
        ...
        bA ~ dnorm(0,10),
        bAR ~ dnorm(0,10),
        sigma ~ dcauchy(0,2) # a weakly regularizing prior for standard deviations
...
```{r 8.7}
output <- capture.output(m8.1stan_4chains <- update( m8.1stan , chains=4 , cores=4 ))
# map2stan doesn't work anymore- use update() here. 
...
str(post)
# Samples are in a list- not in a data.frame (useful for multilevel models)

# For now, if it doesn’t behave like you expect it to, you can coerce it to a data frame 
post<-as.data.frame(post)

# To plot all these samples at once, provided there aren’t too many parameters
pairs(post)

# To display parameter names and parameter correlations
pairs(m8.1stan)
...
show(m8.1stan)
# map2stan does not work anymore and ulam doesn't compute WAIC and DIC 
...
# use bayesplot’s mcmc_trace() to show only trace plots with our custom theme. 
# Note that mcmc_trace() works with data frames, not brmfit objects. There’s a further 
# complication. Recall how we made post (i.e., post <- posterior_samples(b8.1)). 
# Our post data frame carries no information on chains. To retain that information, 
# we’ll need to add an add_chain = T argument to our posterior_samples() function.
...
## Only one chain is shown here. Graphs are not displayed properly. 
...
```{r 8.13, message=FALSE, warning=FALSE}

y <- c(-1,1)
output<- capture.output(m8.2 <- ulam(
    alist(
        y ~ dnorm( mu , sigma ) ,
        mu <- alpha,
        alpha ~ dnorm(-1e20, 1e20),
        sigma ~ dunif(0, 1e20)
), # I add in priors coz ulam shows error without priors
data=list(y=y) , 
start=list(alpha=0,sigma=1) , chains=2 , iter=4000 , warmup=1000 ))

precis(m8.2)
# The mean of −1 and 1 is zero, so we’re hoping to get a mean value for alpha around zero. 
# Instead we get crazy values and implausibly wide confidence intervals. 
# Inference for sigma is no better. The number of effective samples, n_eff, is very small. 
# And Rhat should approach 1.00 in a healthy set of chains. Even a value of 1.01 is suspicious. 
# An Rhat of 1.10 indicates a catastrophe.

## I can't do trace plots with ulam.
## Error: cannot coerce class ‘structure("ulam", package = "rethinking")’ to a data.frame

## Add a very weakly informative prior
output <- capture.output(m8.3 <- ulam(
    alist(
        y ~ dnorm( mu , sigma ) , # y_i ∼ Normal(μ, σ)
        mu <- alpha ,             # μ = α
        alpha ~ dnorm( 1 , 10 ) , # α ∼ Normal(1, 10)
        sigma ~ dcauchy( 0 , 1 )  # σ ∼ HalfCauchy(0, 1)
),
data=list(y=y) , start=list(alpha=0,sigma=1) , 
chains=2 , iter=4000 , warmup=1000 ))

precis(m8.3)
# Much better values with weakly informative priors for alpha and sigma. 
```
...
## Second chain is missing and Ip_ graph is not supposed to show. 
## I unloaded `rethinking` and tried to use `brms` only to solve the problem.
## With only `brms`, I got errors that the functions used in akjurz's codes are deprecated.
## It seems both reference codes from the rethinking 1st edition and akjurz's webpage are outdated.
## So, I will focus on the theory and texts only for now. 
