...
The author gave alternative to falsification- that is to build models. According to the author, "Models can be made into testing procedures&mdash; all statistical tests are also models&mdash; but they can also be used to measure, forecast, and argue." He also mentioned three models that are "in demand in both the social and biological sciences."
...
As explained by the author, Bayesian data analysis uses uncertainty or chance to "describe plausibility of different possibilities," and "...we can use probability theory as a general way to represent plausibility, whether in reference to countable events in the world or rather theoretical constructs like parameters." The author then compared this approach to frequentist approach&mdash; "Bayesian probability is a very general approach to probability...The frequentist approach requires that all probabilities be defined by connection to countable events and their frequencies in very large samples." And the author pointed out "...This leads to frequentist uncertainty being premised on imaginary resampling of data&mdash; if we were to repeat the measurement many many times, we would end up collecting a list of values that will have some pattern to it. It means also that parameters and models cannot have probability distributions, only measurements can. The distribution of these measurements is called a sampling distribution. This resampling is never done, and in general it doesn’t even make sense..." 
...
# Use Laplace's rule of succession- add one to both w and n-w to avoid assigning zero 
# probability, ensure at least there's one w and one n-w, even if none have been observed yet
...
- The loss is proportional to the distance of your decision from the true value
  - Loss <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\propto}" title="" /> d - p, where d is the decision and p is the correct answer or true value 
...
- The probability density of some value y, given a Gaussian (normal) distribution with mean <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> and standard deviation <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />, is:

![](https://latex.codecogs.com/svg.latex?\boldsymbol{p(y|\mu,\sigma)} = \boldsymbol{\frac{1}{\sqrt{2\pi\sigma^2}} \exp\left(- \frac{(y-\mu)^2}{2\sigma^2}\right)})

<br>

- (y − <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" />)^2^ gives the normal distribution its fundamental shape, a quadratic shape. Once you exponentiate the quadratic shape, you get the classic bell curve. The rest of it just scales and standardizes the distribution so that it sums to one, as all probability distributions must. But an expression as simple as exp(−y^2^) yields the Gaussian prototype.
...
- The Gaussian distribution is routinely seen without <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />but with another parameter, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\tau}" title="" /> . The parameter <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\tau}" title="" /> in this context is usually called precision and defined as <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\tau}" title="" /> = 1/<img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />^2^. This change of parameters gives us the equivalent formula (just substitute <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" /> = 1/ <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sqrt\tau}" title="" />
):

![](https://latex.codecogs.com/svg.latex?\boldsymbol{p(y|\mu, \tau)} = \boldsymbol{ \sqrt\frac{\tau}{{2\pi}} \exp\left(- \frac{1}{2}\tau(y - \mu)^2 \right)})
...
<img src="https://latex.codecogs.com/svg.latex?\dpi{110}\inline&space;\dpi{110}\boldsymbol{\begin{align*}outcome_i &\sim Normal(\mu_i, \sigma) \\ \mu_i &= \beta \times predictor_i \\ \beta &\sim Normal(0, 10) \\ \sigma &\sim HalfCauchy(0, 1)\end{align*}}" title="" />
...
Normal(<img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" />, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />)    = normal distribution with mean <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" /> and standard deviation <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />

<img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" />    = mean (<img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" />) of the normal distribution for the i^th^ observation

<img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta\times\boldsymbol{predictor}}" title="" /> = the product of a coefficient (<img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta}" title="" />) and a predictor variable (predictor~i~)
...
<img src="https://latex.codecogs.com/svg.latex?\dpi{110}\inline&space;\dpi{110}\boldsymbol{\begin{align*}w \sim Binomial(n, p)\\ p \sim Uniform(0, 1)\\ \end{align*}}" title="" />
...
![](https://latex.codecogs.com/svg.latex?\textbf{posterior probability of any particular value of p} \boldsymbol{=} \frac{\textbf{likelihood} \boldsymbol{\times} \textbf{prior}}{\textbf{average likelihood}})
...
There are an infinite number of possible Gaussian distributions. Some have small means. Others have large means. Some are wide, with a large <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />. Others are narrow. We want our Bayesian machine to consider every possible distribution, each defined by a combination of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> and <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />, and rank them by posterior plausibility. Posterior plausibility provides a measure of the logical compatibility of each possible distribution with the data and model.

In practice we’ll use approximations to the formal analysis. So we won’t really consider every possible value of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> and <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />.  The “estimate” here will be the entire posterior distribution, not any point within it. And as a result, the posterior distribution will be a distribution of Gaussian distributions. Yes, a distribution of distributions.
...
There are an infinite number of Gaussain distributions , with an infinite number of different means and standard deviations. We’re ready to write down the general model and compute the plausibility of each combination of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> and <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />. To define the heights as normally distributed with a mean <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> and standard deviation <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />, we write:

![](https://latex.codecogs.com/svg.latex?\boldsymbol{{h}_i \sim {Normal} (\mu, \sigma)})
...
Then, we add priors assuming Pr(<img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" />, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />) = Pr(<img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" />) Pr(<img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />):

<img src="https://latex.codecogs.com/svg.latex?\dpi{110}\inline&space;\dpi{110}\boldsymbol{\begin{align*}\boldsymbol{h}_i &\sim \boldsymbol{Normal}(\boldsymbol{\mu}, \boldsymbol{\sigma}) && [\boldsymbol{Likelihood}]\\ \boldsymbol{\mu} &\sim \boldsymbol{Normal}(178, 20) && [\boldsymbol{\mu ~ prior}]\\ \boldsymbol{\sigma} &\sim \boldsymbol{Uniform}(0, 50) && [\boldsymbol{\sigma ~ prior}]\end{align*}}" title="" />
...
<span style="color:brown"><span style="text-decoration:underline">**Sample size and the normality of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />'s posterior**</span>
...
<span style="color:brown; text-decoration:underline">**Use `map` to find the values of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> and <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" /> that maximize the posterior probability:**</span>
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}h_i&\sim&space;Normal(\mu_i,\sigma)&&&[height\sim&space;dnorm(mu,sigma)]\\\mu&\sim&space;Normal(178,20)&&&[mu\sim&space;dnorm(178,20)]\\\sigma&\sim&space;Uniform(0,50)&&&[sigma\sim&space;dunif(0,50)]\\\end{align*}" title="\begin{align*}h_i&\sim Normal(\mu_i,\sigma)&&&[height\sim dnorm(mu,sigma)]\\\mu&\sim Normal(178,20)&&&[mu\sim dnorm(178,20)]\\\sigma&\sim Uniform(0,50)&&&[sigma\sim dunif(0,50)]\\\end{align*}" />
...
The two functions alist and list do the same basic thing: allow you to make a collection of 
arbitrary R objects. They differ in one important respect: list evaluates the code you embed 
inside it, while alist does not. So when you define a list of formulas, you should use alist, 
so the code isn’t executed. But when you define a list of start values for parameters, you 
should use list, so that code like mean(d2$height) will be evaluated to a numeric value.

```

<br>

See the effect of a more information prior for <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" />:
...
<span style="color:purple">***Getting <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" /> right***</span>
...
The numbers in the default precis output aren’t sufficient to describe the quadratic posterior completely. For that, we also require the variance-covariance matrix. We’re interested in correlations among parameters&mdash; we already have their variance in the table above&mdash; so let’s go straight to the correlation matrix:
...
Since the posterior for <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> is a distribution, you can find intervals for it, just like for any posterior distribution. To find the 89% highest posterior density interval of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> at 50 kg, just use the HPDI command as usual:
...
Now we have a distribution of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> for each individual in the original data. We actually want something slightly different: a distribution of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> for each unique weight value on the horizontal axis.
...
(1) Use `link` to generate distributions of posterior values for <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" />. The default behavior of `link` is to use the original data, so you have to pass it a list of new horizontal axis values you want to plot posterior predictions across.

(2) Use summary functions like `mean` or `HPDI` or `PI` to find averages and lower and upper bounds of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> for each value of the predictor variable.
...
Now let's generate an 89% prediction interval for actual heights, not just the average height, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> by incorporating the standard deviation <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" /> and its uncertainty as well. 

Imagine simulating heights. For any unique weight value, you sample from a Gaussian distribution with the correct mean <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> for that weight, using the correct value of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" /> sampled from the same posterior distribution. If you do this for every sample from the posterior, for every weight value of interest, you end up with a collection of simulated heights that embody the uncertainty in the posterior as well as the uncertainty in the Gaussian likelihood.
...
Polynomial regression is one of the ways to model a curved relationship between two variables. In this context, “polynomial” means equations for <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" /> that add additional terms with squares, cubes, and even higher powers of the predictor variable. There’s still only one predictor variable in the model, so this is still a bivariate regression. But the definition of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" /> has more parameters now.

The most common polynomial regression, a parabolic model of the mean:

![](https://latex.codecogs.com/svg.latex?\boldsymbol{\mu_i = \alpha + \beta_1 x_i + \beta_2 x_i^2})
<br>

The above is a parabolic (second order) polynomial. The <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\alpha + \beta_1 x_i}" title="" /> part is the same linear function of x in a linear regression, just with a little “1” subscript added to the parameter name, so we can tell it apart from the new parameter. The additional term uses the square of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{x_i}" title="" /> to construct a parabola, rather than a perfectly straight line. The new parameter <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_2}" title="" /> measures the curvature of the relationship.

The parabolic model of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" /> above is still called a “linear model” of the mean. What “linear” usually means in this context is that <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" /> is a linear function of any single parameter. 
...
To fit the parabolic model, just modify the definition of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" />. Here’s the model (with very weak priors):

<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}h_i&\sim&space;Normal(\mu_i,\sigma)&&&height&\sim&space;dnorm(mu,sigma)\\\mu_i&=\alpha&plus;\beta_1x_i&plus;\beta_2x_i^2&&&mu&\leftarrow&space;a&plus;b1\times&space;weight.s&plus;b2\times&space;weight.s^2\\\alpha&\sim&space;Normal(178,100)&&&a&\sim&space;dnorm(178,100)\\\beta_1&\sim&space;Normal(0,10)&&&b1&\sim&space;dnorm(0,10)\\\beta_2&\sim&space;Normal(0,10)&&&b2&\sim&space;dnorm(0,10)\\\sigma&\sim&space;Uniform(0,50)&&&sigma&\sim&space;dunif(0,50)\\\end{align*}" title="\begin{align*}h_i&\sim Normal(\mu_i,\sigma)&&&height&\sim dnorm(mu,sigma)\\\mu_i&=\alpha+\beta_1x_i+\beta_2x_i^2&&&mu&\leftarrow a+b1\times weight.s+b2\times weight.s^2\\\alpha&\sim Normal(178,100)&&&a&\sim dnorm(178,100)\\\beta_1&\sim Normal(0,10)&&&b1&\sim dnorm(0,10)\\\beta_2&\sim Normal(0,10)&&&b2&\sim dnorm(0,10)\\\sigma&\sim Uniform(0,50)&&&sigma&\sim dunif(0,50)\\\end{align*}}" />
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}h_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_1 x_i + \beta_2 x_i^2 + \beta_3 x_i^3\\\end{align*}" title="" />
...
4E1. <img src="https://latex.codecogs.com/svg.image?\boldsymbol{y_i \sim Normal(\mu, \sigma)}" title="" />
...
4E3. <img src="https://latex.codecogs.com/svg.image?\boldsymbol{Pr(\mu, \sigma|y) = \frac{Normal(y|\mu,\sigma) Normal(\mu|0,10) Uniform(\sigma|0,10) }{\int \int Normal(y|\mu,\sigma) Normal(\mu|0,10) Uniform(\sigma|0,10)d\mu d\sigma}}" title="" />

<br>

4E4. ![](https://latex.codecogs.com/svg.latex?\boldsymbol{\mu_i = \alpha + \beta x_i})
...
4M3. 
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}y_i &\sim Normal(\mu, \sigma)\\\mu &= \alpha + \beta x_i\\\alpha &\sim Normal(0, 50)\\\beta &\sim Normal(0, 10)\\\sigma &\sim Uniform(0, 50)\\\end{align*}" title="" />


<br>

4M4. We don't know the age and gender of students. So, we will use uninformative priors, inclusive school age children to adult. 200 is chosen considering the adult height (may not be necessary?) and mean <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta}" title="" /> is zero considering height will not increase if the students are adult, but <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" /> is 40 to leave room for potential increase in height. 

<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}h_i &\sim Normal(\mu, \sigma)\\\mu &= \alpha + \beta y_i\\\alpha &\sim Normal(0, 200)\\\beta &\sim Normal(0, 40)\\\sigma &\sim Uniform(0, 50)\\\end{align*}" title="" />
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}h_i &\sim Normal(\mu, \sigma)\\\mu &= \alpha + \beta y_i\\\alpha &\sim Normal(120, 20)\\\beta &\sim Normal(6, 10)\\\sigma &\sim Uniform(0, 30)\\\end{align*}" title="" />

<br>

4M6. If the variance  among heights for students of the same age is never more than 64cm:

<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}h_i &\sim Normal(\mu, \sigma)\\\mu &= \alpha + \beta y_i\\\alpha &\sim Normal(120, 20)\\\beta &\sim Normal(6, 10)\\\sigma &\sim Uniform(0, 64)\\\end{align*}" title="" />
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}D_i &\sim Normal(\mu_i, \sigma) \\\mu_i &= \alpha + \beta_A A_i \\\alpha &\sim Normal(10, 10) \\\beta_A &\sim Normal(0, 1) \\\sigma &\sim Uniform(0, 10)\\\end{align*}" title="" />
...
Multivariate regression formulas look a lot like the polynomial models- they add more parameters and variables to the definition of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" />. The strategy is straightforward:
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}D_i &\sim Normal(\mu, \sigma)\ &&&  [likelihood]\\\mu_i &= \alpha + \beta_R R_i + \beta_A A_i\ &&&  [linear~model]\\\alpha &\sim Normal(10, 10)\ &&& [prior~for~\alpha]\\\beta_R &\sim Normal(0, 1)\ &&&  [prior~for~\beta_R]\\\beta_A &\sim Normal(0, 1)\ &&&   [prior~for~\beta_A]\\\sigma &\sim Uniform(0, 10)\ &&&  [prior~for~\sigma]\\\end{align*}" title="" />
...
![](https://latex.codecogs.com/svg.latex?\boldsymbol{\mu_i = \alpha + \beta_1 x_{1i} + \beta_2 x_{2i} + ... + \beta_n x_{ni}})

<br>

In compact form:

![](https://latex.codecogs.com/svg.latex?\boldsymbol{\mu_i = \alpha + \sum_{j = 1}^{n} \beta_j x_{ji}})
...
![](https://latex.codecogs.com/svg.latex?\boldsymbol{m = Xb})
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}D_i &\sim Normal(\mu, \sigma)\ &&&  Divorce &\sim dnorm(mu, sigma)\\\mu_i &= \alpha + \beta_R R_i + \beta_A A_i\ &&&  mu &\leftarrow a + bR^*Marriage.s + bA^*MedianAgeMarriage.s\\\alpha &\sim Normal(10, 10)\ &&&  a &\sim dnorm(10, 10)\\ \beta_R &\sim Normal(0, 1)\ &&& bR &\sim dnorm(0, 1)\\\beta_A &\sim Normal(0, 1)\ &&& bA &\sim dnorm(0, 1)\\\sigma &\sim Uniform(0, 10)\ &&& sigma &\sim dunif(0, 10)\\\end{align*}" title="" />
...
A predictor variable residual is the average prediction error when we use all of the other predictor variables to model a predictor of interest. The benefit of computing these things is that, once plotted against the outcome, we have a bivariate regression of sorts that has already “controlled” for all of the other predictor variables. It just leaves in the variation that is not expected by the model of the mean, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" />, as a function of the other predictors.
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}R_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta A_i\\\alpha &\sim Normal(0, 10)\\\beta &\sim Normal(0, 1)\\\sigma &\sim Uniform(0, 10)\\\end{align*}" title="" />
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}k_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_n n_i + \beta_m log(m_i)\\\alpha &\sim Normal(0, 100)\\\beta_n &\sim Normal(0, 1)\\\beta_m &\sim Normal(0, 1)\\\sigma &\sim Uniform(0, 10)\\\end{align*}" title="" />
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}y_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_1 x_i + \beta_2 x_i\\\end{align*}" title="" />
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}y_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + (\beta_1 + \beta_2) x_i\\\end{align*}" title="" />

<br>

This means the posterior distribution ends up reporting the practically infinite combinations of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_1}" title="" /> and <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_2}" title="" /> that make their sum close to the actual association of x with y.
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}h_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_m m_i\\\alpha &\sim Normal(178, 100)\\\beta_m &\sim Normal(0, 10)\\\sigma &\sim Uniform(0, 50)\\\end{align*}" title="" />
...
The parameter <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_m}" title="" /> influences prediction only for those cases where <img src="https://latex.codecogs.com/svg.image?\boldsymbol{m_i}" title="" /> = 1. When <img src="https://latex.codecogs.com/svg.image?\boldsymbol{m_i}" title="" /> = 0, it has no effect on prediction, because it is multiplied by zero inside the linear model, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\alpha + \beta_m m_i}" title="" />, canceling it out, whatever its value.
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}h_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha_f(1 - m_i) + \alpha_m m_i\\\end{align*}" title="" />
...
Plug in <img src="https://latex.codecogs.com/svg.image?\boldsymbol{m_i}" title="" /> = 0 and <img src="https://latex.codecogs.com/svg.image?\boldsymbol{m_i}" title="" /> = 1 to verify that either <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\alpha_f}" title="" /> or <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\alpha_m}" title="" />, but never both, is turned on for any individual case i. 
...
The mathematical model can be rearranged where the old <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_m}" title="" /> as the difference between the average male and female, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\alpha_m - \alpha_f}" title="" />:

![](https://latex.codecogs.com/svg.latex?\boldsymbol{\mu_i = \alpha_f + (\alpha_m - \alpha_f) m_i})
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}k_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_{NWM} NWM_i + \beta_{OWM} OWM_i + \beta_S S_i\\\alpha &\sim Normal(0.6, 10)\\\beta_{NWM} &\sim Normal(0, 1)\\\beta_{OWM} &\sim Normal(0, 1)\\\beta_S &\sim Normal(0, 1)\\\sigma &\sim Uniform(0, 10)\\\end{align*}" title="" />
...
Design formulas (input format for models) take the expression for <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" /> in the detailed mathematical form of a model and strip out the parameters, leaving only a series of predictor names, separated by + signs. These formulas are sufficient to describe the model’s design, provided all priors are flat.

The linear regression model:

<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}y_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta x_i\\\end{align*}" title="" />


<br>

The corresponding design formula with the leading 1 on the right-hand side indicates the intercept, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\alpha}" title="" />:
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}y_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_x x_i + \beta_z Z_i + \beta_w W_i\\\end{align*}" title="" />

<br>

The corresponding design formula:

<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}y \sim 1 + x + z + w\\\end{align*}" title="" />
...
When you omit the explicit intercept, `lm` assumes you wanted one. If you really do not want an intercept&mdash; equivalent to fixing <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\alpha = 0}" title="" />&mdash; then you can use one of these forms:
...
#### No estimate for <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />

Using `lm` will not provide a posterior distribution for the standard deviation <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />&mdash; it won’t be reported in the table of estimates. If you ask for a summary of the model fit, `lm` will report the “residual standard error,” which is a slightly different estimate of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />, but without any uncertainty information like a standard deviation (or standard error).

Use `map` if you need <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />&mdash; get some sense of the uncertainty of the estimate. As a bonus, you can use informative priors then, as well.

<br>

### Building `map` formulas from `lm` formulas

The `rethinking` package provides a function, `glimmer`, that translates design formulas into `map`&mdash; style model formulas.
...
![\large](https://latex.codecogs.com/svg.latex?\textit{\textbf{What is the value of knowing each predictor, once we already know the other predictors?}})
...
(2) ![](https://latex.codecogs.com/svg.latex?\boldsymbol{\mu_i = \beta_x x_i + \beta_z Z_i})
(4) ![](https://latex.codecogs.com/svg.latex?\boldsymbol{\mu_i = \alpha + \beta_x x_i + \beta_z Z_i})

<br>

> 5E2. Write down a multiple regression to evaluate the claim: Animal diversity is linearly related to latitude, but only after controlling for plant diversity. You just need to write down the model definition.

<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}AD_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_L L_i + \beta_D D_i\\\end{align*}" title="" />
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}T_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_F F_i + \beta_S S_i\\\end{align*}" title="" />
...
When the predictors are fitted separately to the model, the <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta}" title="" /> coefficients for `tree.number` and `factory.number` are 0.52 and -0.47, in relation to the outcome `pm.air`. But when the predictors are fitted together in the model, stronger associations with outcome variable are seen with each of the predictors (0.99 for `tree.number` and -0.97 for `factory.number`). 
...
![\large](https://latex.codecogs.com/svg.latex?\textbf{\textit{Information:} The reduction in uncertainty derived from learning an outcome.})
...
![\large](https://latex.codecogs.com/svg.latex?\mathbf{H(p) = ~- E~log(p_i) = -\sum_{i=1}^{n} p_i ~log(p_i)})
...
![\large](https://latex.codecogs.com/svg.latex?\mathbf{H(p) = ~-(p_1 ~log(p_1) ~+~ p_2 ~log(p_2)) \approx 0.61})
...
The only trick in computing *H* is to deal with the inevitable question of what to do when *p~i~* = 0. The log(0) = −<img src="https://latex.codecogs.com/svg.image?\boldsymbol{\infty}" title="" />, which won’t do. So just assume that 0 log(0) = 0, when you compute *H*. In other words, events that never happen drop out. It may make more sense to just remember that when an event never happens, there’s no point in keeping it in the model.
...
![\large](https://latex.codecogs.com/svg.latex?\textbf{\textit{Divergence:} ~The additional uncertainty induced by using probabilities from one distribution to describe another distribution.})
...
![\large](https://latex.codecogs.com/svg.latex?\mathbf{D_{KL} (p, q) = \sum_i~ p_i ~(~log(p_i) - log(q_i)) = \sum_i~p_i ~log \left(\frac{p_i}{q_i}\right)})
...
![\large](https://latex.codecogs.com/svg.latex?\mathbf{D_{KL} (p, q) = D_{KL} (p, p) = \sum_i ~p_i  ~(~log (p_i) - log(p_i)) = 0})

<br>


There is no additional uncertainty induced when we use a probability distribution to represent itself. But more importantly, as *q* grows more different from *p*, the divergence <img src="https://latex.codecogs.com/svg.image?\boldsymbol{D_{KL}}" title="" /> also grows. Suppose the true target distribution is p = {0.3, 0.7}. Suppose the approximating distribution *q* can be anything from *q* = {0.01, 0.99} to *q* = {0.99, 0.01}. The first of these probabilities, *q~1~*, is displayed on the horizontal axis, and the vertical displays the divergence <img src="https://latex.codecogs.com/svg.image?\boldsymbol{D_{KL}}" title="" />(*p*, *q*). Only exactly where *q* = *p*, at *q~1~* = 0.3, does the divergence achieve a value of zero. Everyplace else, it grows.

Divergence can help us contrast different approximations to *p*. As an approximating function *q* becomes more accurate, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{D_{KL}}" title="" />(*p*, *q*) will shrink. So if we have a pair of candidate distributions, then the candidate that minimizes the divergence will be closest to the target. Since predictive models specify probabilities of events (observations), we can use divergence to compare the accuracy of models.
...
- ***Cross entropy:*** 
    ...
    - <img src="https://latex.codecogs.com/svg.image?\boldsymbol{H(p, q) = ~-\sum_i p_i ~log(q_i)}" title="" />
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}\ D_{KL} (p, q) &= H(p, q) ~- ~ H(p)\\&= ~- \sum_i p_i ~log(q_i) ~-~ \left(- \sum_i p_i ~log(p_i) \right)\\&= ~- \sum_i p_i ~\left( log(q_i) ~-~ log(p_i) \right)\\\end{align*}" title="" />
...
![\large](https://latex.codecogs.com/svg.latex?\mathbf{D(q) = ~-2 \sum_i log(q_i)})
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}y_i &\sim Normal(\mu_i, 1)\\\mu_i &\sim (0.15)x_{1,i} ~-~ (0.4)x_{2,i}\\\end{align*}" title="" />
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}y_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta x_i\\\alpha &\sim Normal(0, 100)\\\beta &\sim Normal(0, 1)\\\sigma &\sim Uniform(0, 10)\\\end{align*}" title="" />

<br>

Assume, as is good practice, that the predictor x is standardized so that its standard deviation is 1 and its mean is zero. Then the prior on <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\alpha}" title="" /> is a nearly flat prior that has no practical effect on inference. 

But the prior on <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta}" title="" /> is narrower and is meant to regularize. The prior <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta}" title="" /> ∼ Normal(0, 1) says that, before seeing the data, the machine should be very skeptical of values above 2 and below −2, as a Gaussian prior with a standard deviation of 1 assigns only 5% plausibility to values above and below 2 standard deviations. Because the predictor variable x is standardized, you can interpret this as meaning that a change of 1 standard deviation in x is very unlikely to produce 2 units of change in the outcome.
...
![\large](https://latex.codecogs.com/svg.latex?\mathbf{AIC = D_{train} ~+~ 2p})
...
- ***Deviance Information Criterion (DIC)***
    - accommodates informative priors, but still assumes that the posterior is multivariate Gaussian and that <img src="https://latex.codecogs.com/svg.image?\boldsymbol{N \gg k}" title="" />
...
![\large](https://latex.codecogs.com/svg.latex?\mathbf{DIC = \overline{D} + (\overline{D} - \hat{D}) = \overline{D} + p_D})
...
The second piece of WAIC is the effective number of parameters *p~{WAIC}~*. 

![\large](https://latex.codecogs.com/svg.latex?\mathbf{p_{WAIC} = \sum_{i=1}^N ~V(y_i)})
...
![\large](https://latex.codecogs.com/svg.latex?\mathbf{WAIC = ~-2 (lppd~-~ p_{WAIC})})
...
Fitting the four models, using map, is straightforward. The only thing that differs among these models is the equation for <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> (mu). 

A way to constrain the standard deviation of the outcome, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />, to be positive is to estimate the logarithm of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />, which can be any real number. Then inside the likelihood function we just exponentiate it. Since *exp(x) > 0* for any real number *x*, this effectively bounds <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" /> to positive reals.
...
rethinking::WAIC( m6.13 )
rethinking::WAIC( m6.12 )
rethinking::WAIC( m6.11 )
...
- First, regard WAIC as the expected deviance of a model on future data. That is to say that WAIC gives us an estimate of E(D~{test}~). 
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}Y_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_R R_i + \beta_A A_i\\\end{align*}" title="" />
...
Now you want to allow the relationship between Y and R to vary as a function of A. Within the model, this relationship is measured by the slope <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_R}" title="" />. Following the same strategy of replacing parameters with linear models, the most straightforward way to make <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_R}" title="" /> depend upon *A* is just to define the slope <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_R}" title="" /> as a linear model itself, one that includes *A*.

<br>

<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}Y_i &\sim Normal(\mu_i, \sigma) &&&    [likelihood]\\\mu_i &= \alpha + \gamma_i R_i + \beta_A A_i &&&   [linear~model~of~\mu]\\\gamma_i &= \beta_R + \beta_{AR} A_i &&&    [linear~model~of~slope]\\\end{align*}" title="" />

<br>

By defining the relationship between GDP and ruggedness in this way, you are explicitly modeling the hypothesis that the slope between GDP and ruggedness depends&mdash; is *conditional*&mdash; upon whether or not a nation is in Africa. The parameter <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_{AR}}" title="" /> defines the strength of this dependency. If you set <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_{AR}}" title="" /> = 0, then you get the previous model back. If instead <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_{AR}}" title="" /> > 0, then African nations have a more positive slope between GDP and ruggedness. If <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_{AR}}" title="" /> < 0, African nations have a more negative slope. For any nation not in Africa, A~i~ = 0 and so the interaction parameter <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_{AR}}" title="" /> has no effect on prediction for that nation.
...
Instead of a model definition with more than one linear model, as you saw above, it’s conventional to multiply out any interaction effects, so that there is only one linear model. The equation for <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\gamma}" title="" /> can be substituted into the second line and expanded.

<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}Y_i &\sim Normal(\mu_i, \sigma) &&&    [likelihood]\\\mu_i &= \alpha + \beta_R R_i + \beta_{AR} A_i R_i + \beta_A A_i &&&    [linear~model~of~\mu]\\\end{align*}" title="" />
...
Now the change in <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" /> that results from a unit change in *R~i~* is given by <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\gamma_i}" title="" />. And since <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\gamma_i}" title="" /> is a function of three things&mdash; <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_R}" title="" />, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_{AR}}" title="" />, and *A~i~*&mdash; we have to know all three in order to know the influence of *R~i~* on the outcome. The only time the slope <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_R}" title="" /> has its old meaning is when *A~i~ = 0*, which makes <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\gamma_i = \beta_R}" title="" />. Otherwise, to compute the influence of *R~i~* on the outcome, we have to simultaneously consider two parameters and another predictor variable.
...
Since <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\gamma}" title="" /> (gamma) doesn’t appear in this table&mdash; it wasn’t estimated&mdash; we have to compute it ourselves. 
...
![\large ](https://latex.codecogs.com/svg.latex?\boldsymbol{\gamma = \beta_R + \beta_{AR}(0) = -0.18})
...
Anything calculated using parameters has a distribution. Since <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\gamma}" title="" /> depends upon parameters, and those parameters have a posterior distribution, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\gamma}" title="" /> must also have a posterior distribution.
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}Y_i &\sim Normal(\mu_i, \sigma) &&&    [likelihood]\\\mu_i &= \alpha + \gamma_i R_i + \beta_A A_i &&&    [linear~model~of~\mu]\\\gamma_i &\sim \beta_R + \beta_{AR} A_i &&&   [linear~model~of~slope]\\\end{align*}" title="" />

<br>

Let’s expand <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\gamma_i}" title="" /> into the expression for <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" />:

<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}\mu_i &= \alpha + (\beta_R + \beta_{AR} A_i) R_i + \beta_A A_i\\ &= \alpha + \beta_R R_i + \beta_{AR} A_i R_i + \beta_A A_i\\\end{align*}" title="" />

<br>

Now factor together the terms with *A~i~* in them:

![\large ](https://latex.codecogs.com/svg.latex?\boldsymbol{\mu_i = \alpha + \beta_R R_i + \underbrace{(\beta_A + \beta_{AR}R_i)}_G A_i})

<br>

The term labeled *G* looks a lot like <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\gamma_i}" title="" /> in the original form. This is the same model of <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu_i}" title="" />, but re-expressed so that the linear interaction applies to *A~i~*&mdash; prove that linear interactions are symmetric. Within the model, there’s no basis to prefer one interpretation over the other, because in fact they are the same interpretation. But when we reason causally about models, our minds tend to prefer one interpretation over the other, because it’s usually easier to imagine manipulating one of the predictor variables instead of the other&mdash; hard to imagine manipulating which continent a nation is on, but it's easy to imagine manipulating terrain ruggedness.
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}B_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_W W_i + \beta_S S_i\\\end{align*}" title="" />

<br>

And the full interaction likelihood is:

<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}B_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_W W_i + \beta_S S_i + \beta_{WS} W_i S_i\\\end{align*}" title="" />
...
Estimation failed with the un-centered predictor variables, because there was a long way to go from the `start` values you provided to map and the `MAP` values it was seeking. When the predictors are centered, the MAP value for <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\alpha}" title="" /> is just the empirical mean for the outcome. That is also the `start` value given to `map`. With un-centered predictors, the MAP value for <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\alpha}" title="" /> lies a long way from the empirical mean. Hence, the long search that failed.

<br>

#### Estimates changed less across models

In the un-centered models, the interaction effect is applied to every case, and so none of the parameters in <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\mu}" title="" /> makes sense alone. This is because neither of the predictors in those models, `shade` and `water`, are ever zero. As a result the interaction parameter always factors into generating a prediction. Consider for example a tulip at the average moisture and shade levels, 2 in each case. The expected blooms for such a tulip is:

![\large ](https://latex.codecogs.com/svg.latex?\boldsymbol{\mu_i|_{S_i=2, W_i=2} = \alpha + \beta_W(2) + \beta_S (2) + \beta_{WS}(2 \times 2)})
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}y_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_x x_i + \beta_z z_i + \beta_{xz} x_i z_i\\\end{align*}" title="" />
...
If you do not want the main effects in the model, you must explicitly subtract them from the formula. For example, here’s a model with an interaction, but without one of the main effects (when you know a priori that there is no direct effect of *z* on the outcome, when <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\beta_z}" title="" /> = 0 by assumption):

<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}y_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_x x_i + \beta_{xz} x_i z_i\\\end{align*}" title="" />
...
<img src="https://latex.codecogs.com/svg.latex?\inline&space;\dpi{110}\boldsymbol{\begin{align*}y_i &\sim Normal(\mu_i, \sigma)\\\mu_i &= \alpha + \beta_x x_i + \beta_z z_i + \beta_w w_i + \beta_{xz} x_i z_i + \beta_{xw} x_i w_i + \beta_{zw} z_i w_i + \beta_{xzw} x_i z_i w_i \\\end{align*}" title="" />
...
***Markov chain Monte Carlo (MCMC) estimation*** is the estimation of posterior probability distributions using a stochastic process. It has the ability to directly estimate models, such as the generalized linear and multilevel models, which routinely produce non-Gaussian posterior distributions. 

<br>

## Good King Markov and his island kingdom

King Markov was a benevolent autocrat of an island kingdom,a circular archipelago, with 10 islands. Each island was neighbored by two others, and the entire archipelago formed a ring. The islands were of different sizes, and so had different sized populations living on them. The second island was about twice as populous as the first, the third about three times as populous as the first, and so on, up to the largest island, which was 10 times as populous as the smallest.

The king should visit each island in proportion to its population size, visiting the largest island 10 times as often as the smallest, for example. Also, since the archipelago was a ring, the King insisted that he only move among adjacent islands, to minimize time spent on the water. 
...
```{r 8.1, fig.height=6, fig.width=10}

## Simulate King Markov’s journey (Use akjurz's codes)
...
grid.arrange(d1, d2, ncol = 2)
# Results of the king following the Metropolis algorithm. The left-hand plot shows 
# the king’s position (vertical axis) across first 100 weeks (horizontal axis). 
# In any particular week, it’s nearly impossible to say where the king will be. 

# The right-hand plot shows the long-run behavior of the algorithm, as the time spent on 
# each island turns out to be proportional to its population size.

# The algorithm will still work in this way, even if we allow the king to be equally likely 
# to propose a move to any island from any island, not just among neighbors. The algorithm 
# would also work for any size archipelago, even if the king didn’t know how many islands 
# were in it. All he needs to know at any point in time is the population of the current 
# island and the population of the proposal island.
...
The goal of general **Metropolis Algorithm**, an example of Markov chain Monte Carlo, is to draw samples from an unknown and usually complex target distribution, like a posterior probability distribution.
...
Provided the way we choose our proposed parameter values at each step is symmetric&mdash; so that there is an equal chance of proposing from A to B and from B to A&mdash; then the Metropolis algorithm will eventually give us a collection of samples from the joint posterior.

<br>

<spans style="color:brown"><span style="text-decoration:underline">**Two of the most important in contemporary Bayesian inference**</span>

- Gibbs sampling

- Hamiltonian (aka Hybrid, aka HMC) Monte Carlo

<br>

### Gibbs sampling

The Metropolis algorithm works whenever the probability of proposing a jump to B from A is equal to the probability of proposing A from B, when the proposal distribution is symmetric. There is a more general method, known as Metropolis-Hastings, that allows asymmetric proposals. 

An algorithm that allows asymmetric proposals is useful because:

- it makes it easier to handle parameters, like standard deviations, that have boundaries at zero

- we can acquire an equally good image of the posterior distribution in fewer steps

Gibbs sampling is a variant of the Metropolis-Hastings algorithm that uses clever proposals and is therefore more efficient&mdash; you can get a good estimate of the posterior from Gibbs sampling with many fewer samples than a comparable Metropolis approach. The improvement arises from ***adaptive proposals*** in which the distribution of proposed parameter values adjusts itself intelligently, depending upon the parameter values at the moment. Gibbs sampling computes these adaptive proposals using particular combinations of prior distributions and likelihoods known as *conjugate pairs*.

Some limitations of Gibbs sampling:

- Some conjugate priors seem silly, and choosing a prior so that the model fits efficiently isn’t really a strong argument from a scientific perspective.

- As models become more complex and contain hundreds or thousands or tens of thousands of parameters, Gibbs sampling becomes shockingly inefficient.

<br>

### Hamiltonian Monte Carlo

Hamiltonian Monte Carlo (or Hybrid Monte Carlo, HMC) adopts Jaynes’ principle that, *"whenever there is a randomized way of doing something, then there is a non-randomized way that delivers better performance but requires more thought."*

HMC is much more computationally costly than are Metropolis or Gibbs sampling&mdash; both highly random procedures. But its proposals are typically much more efficient&mdash; doesn’t need as many samples to describe the posterior distribution, outshine other algorithms as models become more complex (thousands or tens of thousands of parameters). HMC outperforms Metropolis and Gibbs sampling, but it is not a universal solution to all MCMC problems.

Monty’s kingdom is not a discrete set of islands. Instead, it is a continuous territory stretched out along a narrow valley. Monty's advisor, Hamilton realized that a much more efficient way to visit the citizens in the continuous Kingdom is to travel back and forth along its length. In order to spend more time in densely settled areas, they should slow the royal vehicle down when houses grow more dense. Likewise, they should speed up when houses grow more sparse. This strategy requires knowing how quickly population density is changing, at their current location. But it doesn’t require remembering where they’ve been or knowing the population distribution anyplace else. And a major benefit of this strategy compared to that of Metropolis is that the King makes a full sweep of the kingdom before revisiting anyone.
...
Some limitations of HMC:

- HMC requires continuous parameters. It can’t glide through a discrete parameter. 

- it needs to be tuned to a particular model and its data. That’s where an engine like Stan (mc-stan.org) comes in. Stan automates much of that tuning.

<br>

## Easy HMC: `map2stan`

`map2stan` compiles lists of formulas, like the lists you’ve been using so far to construct map estimates, into Stan HMC code. To use `map2stan`, you need to preprocess any variable transformations, and you need to construct a clean data frame with only the variables you will use.

```{r 8.2}

# load the data and reduce it down to cases (nations) that have the outcome variable of interest
data(rugged)
d <- rugged
d$log_gdp <- log(d$rgdppc_2000)
dd <- d[ complete.cases(d$rgdppc_2000) , ]

# Fit the interaction model which  aims to predict log-GDP with terrain ruggedness, 
# continent, and the interaction of the two
m8.1 <- rethinking::map(
    alist(
        log_gdp ~ dnorm( mu , sigma ) ,
...
But now we’ll also fit this model using Hamiltonian Monte Carlo. This means there will be no more quadratic approximation&mdash; if the posterior distribution is non-Gaussian, then we’ll get whatever non-Gaussian shape it has. You can use exactly the same formula list as before, but you need to do two additional things.

(1) Preprocess all variable transformations.

      - If the outcome is transformed somehow, like by taking the logarithm, then do this before fitting the model by constructing a new variable in the data frame. 
        
      - Likewise, if any predictor variables are transformed, including squaring and cubing and such to build polynomial models, then compute these transformed values before fitting the model.
...
## Use akjurz's codes (ulam stops working today, so I changed all ulam codes, so doc render)
# Get samples from the posterior distribution using HMC
output <- capture.output(b8.1 <-
  brm(data = dd, family = gaussian,
      log_gdp ~ 1 + rugged + cont_africa + rugged:cont_africa,
      prior = c(prior(normal(0, 100), class = Intercept),
                prior(normal(0, 10), class = b),
                prior(cauchy(0, 2), class = sigma)),
      seed = 8)) # a weakly regularizing prior for standard deviations

# The uniform prior on sigma has been changed to a half-Cauchy prior. (cauchy)
# The Cauchy distribution is a useful thick-tailed probability distribution 
# related to the Student t distribution.

print(b8.1)
## The brms::brm() function defaults to chains = 4. It also defaults to cores = 1, 
## iter = 2000, warmup = 1000. 
## McElreath’s rethinking::precis() returns highest posterior density intervals (HPDIs) 
## when summarizing map2stan() models. Not so with brms. If you want HPDIs, 
## you’ll have to use the convenience functions from the tidybayes package.
...
# The interval boundaries in the table are highest posterior density intervals (HPDI), 
# not ordinary percentile intervals (PI).

# n_eff and Rhat provide MCMC diagnostic criteria, to help you tell how well estimation worked.
...
### Sampling agian, in parallel

Once you have compiled your Stan model, you can draw more samples from it anytime, running as many independent Markov chains as you like. And you can easily parallelize those chains, as well. To run four independent Markov chains for the model above, and to distribute them across separate processors in your computer, just pass the previous fit back to map2stan:

```{r 8.7, message=FALSE, warning=FALSE}
## Here we sample in parallel by adding cores = 4.
output <- capture.output(b8.1_4chains_4cores <- update(b8.1, 
         cores = 4))

## This model sampled so fast that it really didn’t matter if we sampled in 
## parallel or not. It will for others.
...
## Unlike the way rethinking’s extract.samples() yields a list, 
## brms’s posterior_samples() returns a data frame.
post <- posterior_samples(b8.1)
str(post)

## As with McElreath’s rethinking, brms allows users to put the post data frame 
## or the brmsfit object directly in pairs().

# To plot all these samples at once, provided there aren’t too many parameters
pairs(b8.1,
      off_diag_args = list(size = 1/5, alpha = 1/5))
# Pairs plot of the samples produced by Stan. The diagonal shows a density estimate for 
# each parameter. Below the diagonal, correlations between parameters are shown.
...
If you have the samples from the posterior and you know the model, you can do anything: simulate predictions, compute differences between parameters, and calculate DIC and WAIC.

```{r 8.11}

## Use brms (copy Akjurz's codes)
# Run HMC model
output <- capture.output(b8.1 <-
  brm(data = dd, family = gaussian,
      log_gdp ~ 1 + rugged + cont_africa + rugged:cont_africa,
...
# use loo (leave-one-out) cross-validation
(l_b8.1 <- loo(b8.1))

# And the recommended workflow since brms version 2.8.0 is to save the information 
# criteria information with your brm() fit objects with the add_criterion() function.
...
A trace plot&mdash; the most broadly useful tool for diagnosing malfunction&mdash; merely plots the samples in sequential order, joined by a line. Looking at the trace plot of each parameter can help to diagnose many common problems. A trace plot isn’t the last thing analysts do to inspect MCMC output. But it’s nearly always the first.

```{r 8.12}

# In the terrain ruggedness example, the trace plot shows a very healthy chain.
plot(b8.1) # m8.1stan doesn't work

## Use akjurz's codes to do trace plots
p1 <-
  as_draws_df(b8.1) %>% 
  mcmc_trace(pars = vars(0:sigma),
             linewidth = 0.25)
...
# The bayesplot package offers a variety of diagnostic plots. Here we make 
# autocorrelation plots for all model parameters, one for each HMC chain.
mcmc_acf(post, 
         pars = c("b_Intercept", "b_rugged", "b_cont_africa", 
                  "b_rugged:cont_africa", "sigma"),
         lags = 5) +
  scale_color_ipsum() +
  theme_ipsum()
# That’s just what we like to see–nice L-shaped autocorrelation plots. Those are 
# the kinds of shapes you’d expect when you have reasonably large effective samples.
## Only one chain is displayed here and Ip_ (is that a graph for prior?) shouldn't be showing.

```

Typically we look for two things in these trace plots: stationarity and good mixing. Stationarity refers to the path staying within the posterior distribution. Notice that these traces, for example, all stick around a very stable central tendency, the center of gravity of each dimension of the posterior. Another way to think of this is that the mean value of the chain is quite stable from beginning to end.

A well-mixing chain means that each successive sample within each parameter is not highly correlated with the sample before it. Visually, you can see this by the rapid zig-zag motion of each path, as the trace traverses the posterior distribution without getting mired anyplace.

<br>

<span style="color:brown"><span style="text-decoration:underline">**Raw stan model code**</span>

```{r model_block, eval=FALSE, echo=TRUE}

## Stan code
# It is essentially the formula list you provided to map2stan, but in reverse order. 
# The first line, vector[N] mu; names a symbol to hold the linear model. The rest of 
# the code reiterates your formulas, beginning with priors and then computing the 
# value of mu for each observation (nation). Finally, the last line comprises the 
# likelihood. Given this kind of model definition, Stan will define and sample from 
...
- When deciding whether the chains are valid, you need more than one chain. 
    - Using 3 or 4 chains is conventional, and quite often more than enough to reassure us that the sampling is working properly.
    
\n

- When you begin the final run that you’ll make inferences from, you only really need one chain. But using more than one chain is fine, as well. It just doesn’t matter, once you’re sure it’s working.
    - More chains cost you an extra samples of `warmup` that just get thrown away. And since warmup is typically the slowest part of the chain, these extra samples cost a disproportionate amount of your computer’s time.
    - On the other hand, if you run the chains on different computers or processor cores within a single computer, then you might prefer more chains, because you can spread the load and finish the whole job faster.
    
\n

*For typical regression models, you can live by the motto four short chains to check, one long chain for inference.* (Things may still go wrong) 

<br>

<span style="color:brown"><span style="text-decoration:underline">**Convergence diagnostics**</span>

- The default diagnostic output from Stan includes two metrics, `n_eff` and `Rhat`. 
    - `n_eff` is a measure of the effective number of samples. 
        - When `n_eff`is much lower than the actual number of iterations (minus warmup) of your chains, it means the chains are inefficient, but possibly still okay. 
    - `Rhat` is the Gelman-Rubin convergence diagnostic, <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\hat{R}}" title="" />. 
        -  When Rhat is above 1.00, it usually indicates that the chain has not yet converged, and probably you shouldn’t trust the samples. If you draw more iterations, it could be fine, or it could never converge. For example, Rhat can reach 1.00 even for an invalid chain. So view it perhaps as a signal of danger, but never of safety. For conventional models, these metrics typically work well.

<br>

### Taming a wild chain

One common problem with some models is that there are broad, flat regions of the posterior density. This happens most often, as you might guess, when one uses flat priors. The problem this can generate is a wild, wandering Markov chain that erratically samples extremely positive and extremely negative parameter values.

<br>

```{r akjurz_8.13}

## Use akjurz's codes using brms
# As with rethinking, brms can take data in the form of a list. Recall however, 
# that in order to specify starting values, you need to specify a list of lists 
# with an inits argument rather than with start.
output <- capture.output(b8.2 <-
...
print(b8.2)
# Ludicrous numbers, super wide CI and Rhat is higher than 1 (1.23)

## With reasonable priors
output <- capture.output(b8.3 <-
  brm(data = list(y = c(-1, 1)), 
      family = gaussian,
      y ~ 1,
...
# Diagnosing and healing a sick Markov chain. 
# Top row: Trace plot from two independent chains defined by model b8.2. These chains 
# are not stationary and should not be used for inference. 
# Bottom row: Adding weakly informative priors (see b8.3) clears up the condition right away. 
# These chains are fine to use for inference.

# Compare posterior and priors for model b8.3
post <- as_draws_df(b8.3)
#post %>%
  #select(b_Intercept) %>%
 d1 <- ggplot(post, aes(x = b_Intercept)) +
...
# Prior (dashed) and posterior (solid black) for the model with weakly informative priors.
# Even with only two observations, the likelihood easily overcomes these priors. 
# Yet the model cannot be successfully estimated without them.
...
<span style="color:brown"><span style="text-decoration:underline">**Cauchy distribution**</span>

-  The Cauchy (ko-shee) distribution gives the distribution of the ratio of two random Gaussian draws.  
        - Its parameters are a location <img src="https://latex.codecogs.com/svg.image?\boldsymbol{x_0}" title="" /> and a scale <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\gamma}" title="" />. The location says where the center is, and the scale defines how stretched out the distribution is. 
        - Cauchy has no defined mean nor variance, so the location and scale are not its mean and standard deviation. The reason the Cauchy has no mean and variance is that it is a very thick-tailed distribution. At any moment in a Cauchy sampling process, it is possible to draw an extreme value that overwhelms all of the previous draws. The consequence of this fact is that the sequence never converges to a stable mean and variance. It just keeps moving.
        
\n

- Half-Cauchy is a Cauchy defined over the positive reals only. This is because they are applied to a parameter, usually <img src="https://latex.codecogs.com/svg.image?\boldsymbol{\sigma}" title="" />, that is strictly positive.

```{r 8.16}
# The code below samples 10,000 values from a Cauchy distribution. 
# Then it computes and plots the running mean at each sample. 
# Run this simulation a few times to see how the trace of the mean is highly unpredictable.

y <- rcauchy(1e4,0,5)
mu <- sapply( 1:length(y) , function(i) sum(y[1:i])/i )
plot(mu,type="l")
# The distribution of mu changes significantly with each run. 
```
...
The non-identifiable parameters (created by higly correlated predictors) can be identified by using a little prior information.

```{r 8.17, fig.height=5}

# To construct a non-identifiable model, we first simulate 100 observations from 
# a Gaussian distribution with mean zero and standard deviation 1.
y <- rnorm( 100 , mean=0 , sd=1 )

# Fit the model (use akjurz's codes)
output <- capture.output(b8.4 <-
  brm(data = list(y           = y,
                  intercept_1 = 1,
                  intercept_2 = 1), 
      family = gaussian,
...
print(b8.4)
# The estimates look suspicious, and the n_eff and Rhat values are terrible. 
# The means for a1 and a2 are almost exactly the same distance from zero, 
# but on opposite sides. And the standard deviations of the chains are massive. 
# This is of course a result of the fact that we cannot simultaneously estimate 
# a1 and a2, but only their sum.

## Use weak priors
output <- capture.output(b8.5 <-
  brm(data = list(y           = y,
                  intercept_1 = 1,
                  intercept_2 = 1),
      family = gaussian,
...
print(b8.5)
# The estimates for a1 and a2 are better identified now.
...
## Top row: A chain with wandering parameters, b8.4. 
## Bottom row: Same model but now with weakly informative priors, b8.5.
# With flat priors sampling may take 8 times as long as it does for model with weak priors. 
# Often, a model that is very slow to sample is under-identified. This is an aspect of the 
# `folk theorem of statistical computing`: When you are having trouble fitting a model, 
# it often indicates a bad model.
...
Gibbs sampling allows asymmetric proposal distributions, making handling of parameters easier and requiring fewer steps (and samples) to generate a good estimate of posterior distribution. 

Gibbs sampling is inefficient for complex models with many parameters, and it requires combinations of priors and likelihoods known as conjugate pairs. 

<br>

> 8E3. Which sort of parameters can Hamiltonian Monte Carlo not handle? Can you explain why?

Hamiltonian Monte Caro cannot handle discrete parameters as it requires continuous parameters to glide through the log-posterior smoothly, and it makes use of differential equations that apply to continuous variables. 
...
The effective number of samples is an estimate of the number of independent samples from the posterior distribution that would provide similar information as the actual number of samples generated by simulations. 
...
output <- capture.output(goodplot <-
  brm(data = list(y = c(-1, 1)), 
      family = gaussian,
      y ~ 1,
      prior = c(prior(uniform(0, 10), class = Intercept),
...
output <- capture.output(badplot <-
  brm(data = list(y = c(-1, 1)), 
      family = gaussian,
      y ~ 1,
      prior = c(prior(uniform(-1e20, 1e20), class = Intercept),
                prior(uniform(0, 1e20), class = sigma)),
      inits = list(list(Intercept = 0, sigma = 1),
                   list(Intercept = 0, sigma = 1)),
      iter = 4000, warmup = 1000, chains = 2,
      seed = 8))
p1 <-
  as_draws_df(goodplot) %>% 
...
(
  ...
  plot_annotation(subtitle = "Good trace plot (Top) and Bad trace plot (Bottom)")
```

The chains in good trace plot for a Markov chain (top row) are stationary and well-mixing, whereas the chains in the bad trace plot (bottom) are not converging to posterior distribution&mdash; chain 1 wanders off to extreme values and chain 2 is completely flat. 
...
output <- capture.output(exponential <-
  brm(data = dd, family = gaussian,
      log_gdp ~ 1 + rugged + cont_africa + rugged:cont_africa,
      prior = c(prior(normal(0, 100), class = Intercept),
                prior(normal(0, 10), class = b),
...
curve( dcauchy(x,0,2) , from=0 , to=10 ,
    ...
    main = "Comparison of Sigmas for Prior Distributions" )
curve( dunif(x,0,10) , add=TRUE , col="red" )
curve( dexp(x,1) , add=TRUE , col="blue" )
legend("topright", legend = c("Cauchy (scale = 2)", 
                              "Cauchy (scale = 10)", "Exponential (rate = 1)"), 
       col = c("black", "red", "blue"), lty = 1)

# Compare sigmas for posterior distribution
sigma_o <- as_draws_df(b8.1)
sigma_u <- as_draws_df(uniform)
sigma_e <- as_draws_df(exponential)

dens( sigma_o[[5]] , xlab="sigma" , xlim=c(0.5,1.5), ylim=c(0,8),
      main = "Comparison of Sigmas for Posterior Distributions")
dens( sigma_u[[5]] , add=TRUE , col = "red" )
dens( sigma_e[[5]] , add=TRUE , col = "blue" )
legend("topright", legend = c("Cauchy (scale = 2)", "Cauchy (scale = 10)", 
                              "Exponential (rate = 1)"), 
       col = c("black", "red", "blue"), lty = 1)
...
I used `uniform` and `exponential` instead of `dunif` and `dexp`, since I'm using `brms`. The different priors do not have any detectable influence on the posterior distribution, as seen in the `print` results and figures.

<br>

<span style="color:gray">***Given answer:***</span> 

All three posterior distributions are very similar. Why? There’s a lot of data, relative to the difference in priors.
...
# Stronger prior with cauchy
output <- capture.output(uniform1 <-
  brm(data = dd, family = gaussian,
      log_gdp ~ 1 + rugged + cont_africa + rugged:cont_africa,
      prior = c(prior(normal(0, 100), class = Intercept),
                prior(normal(0, 10), class = b),
                prior(cauchy(0, 1), class = sigma)), # weaker prior with bigger sd
      seed = 8))

print(uniform1)

# Stronger prior with exponential
output <- capture.output(exponential2 <-
  brm(data = dd, family = gaussian,
      log_gdp ~ 1 + rugged + cont_africa + rugged:cont_africa,
      prior = c(prior(normal(0, 100), class = Intercept),
                prior(normal(0, 10), class = b),
                prior(exponential(10), class = sigma)), # stronger prior with bigger expo
      seed = 8))
...
p2 <-
  as_draws_df(uniform1) %>% 
  mcmc_trace(pars = vars(b_Intercept:sigma),
             linewidth = 0.25)

p3 <-
  as_draws_df(exponential2) %>% 
  mcmc_trace(pars = vars(b_Intercept:sigma),
             linewidth = 0.25)
...
curve( dexp(x,10) , add=TRUE , col="blue" )
legend("topright", legend = c("Cauchy (scale = 2)", 
                              "Cauchy (scale = 1)", "Exponential (rate = 10)"), 
       col = c("black", "red", "blue"), lty = 1)

# Compare sigmas for posterior distribution
sigma <- as_draws_df(b8.1)
sigma_1 <- as_draws_df(uniform1)
sigma_2 <- as_draws_df(exponential2)

dens( sigma[[5]] , xlab="sigma" , xlim=c(0.5,1.5), ylim=c(0,10),
      main = "Comparison of Sigmas for Posterior Distributions")
dens( sigma_1[[5]] , add=TRUE , col = "red" )
dens( sigma_2[[5]] , add=TRUE , col = "blue" )
legend("topright", legend = c("Cauchy (scale = 2)", "Cauchy (scale = 1)", 
                              "Exponential (rate = 10)"), 
       col = c("black", "red", "blue"), lty = 1)

```

Not very different with slightly stronger priors. Set much stronger prior for exponential:

```{r 8M2.1, fig.height=8}
...
output <- capture.output(uniform_new <-
  brm(data = dd, family = gaussian,
      log_gdp ~ 1 + rugged + cont_africa + rugged:cont_africa,
      prior = c(prior(normal(0, 100), class = Intercept),
                prior(normal(0, 10), class = b),
...
output <- capture.output(exponential_new <-
  brm(data = dd, family = gaussian,
      log_gdp ~ 1 + rugged + cont_africa + rugged:cont_africa,
      prior = c(prior(normal(0, 100), class = Intercept),
                prior(normal(0, 10), class = b),
...
p2 <-
  as_draws_df(uniform_new) %>% 
  mcmc_trace(pars = vars(b_Intercept:sigma),
             linewidth = 0.25)

p3 <-
  as_draws_df(exponential_new) %>% 
  mcmc_trace(pars = vars(b_Intercept:sigma),
             linewidth = 0.25)
...
curve( dcauchy(x,0,2) , from=0 , to=10 ,
    xlab="sigma" , ylab="Density" , ylim=c(0,1),  
    main = "Comparison of Sigmas for Prior Distributions" )
curve( dcauchy(x,0,0.1) , add=TRUE , col="red" )
curve( dexp(x,100) , add=TRUE , col="blue" )
legend("topright", legend = c("Cauchy (scale = 2)", 
                              "Cauchy (scale = 0.1)", "Exponential (rate = 100)"), 
       col = c("black", "red", "blue"), lty = 1)

# Compare sigmas for posterior distribution
sigma_original <- as_draws_df(b8.1)
sigma_uniform <- as_draws_df(uniform_new)
sigma_exponential <- as_draws_df(exponential_new)

dens( sigma_original[[5]] , xlab="sigma" , xlim=c(0.5,1.5), ylim=c(0,13),
      main = "Comparison of Sigmas for Posterior Distributions")
dens( sigma_uniform[[5]] , add=TRUE , col = "red" )
dens( sigma_exponential[[5]] , add=TRUE , col = "blue" )
legend("topright", legend = c("Cauchy (scale = 2)", "Cauchy (scale = 0.1)", 
                              "Exponential (rate = 100)"), 
       col = c("black", "red", "blue"), lty = 1)
...
# Function to fit model and calculate effective sample size
fit.model.function <- function(warmup) {
  # Fit the model
  output <- capture.output(model <- brm(data = list(y = c(-1, 1)), 
               family = gaussian,
               y ~ 1,
               prior = c(prior(normal(0, 10), class = Intercept),
                         prior(cauchy(0, 1), class = sigma)),
               inits = list(list(Intercept = 0, sigma = 1),
                            list(Intercept = 0, sigma = 1)),
               iter = 4000, warmup = warmup, chains = 2,
               seed = 8))
...
# Loop through each warm-up value and fit the model
output <- capture.output(ess_values <- sapply(warmup_values, function(warmup) {
  fit.model.function(warmup)
}))

ess_values

## A warmup value of 5 is enough for this model. 
...
output <- capture.output(mp <-
  brm(data = list(y = 1), 
      ...
      inits = list(list(Intercept = 0, sigma = 1),
                   list(Intercept = 0, sigma = 1)),
      iter = 1e4, warmup = 100, chains = 2,
      seed = 8))

print(mp)

## Here the results show Bulk- and Tail-ESS but I will calculate n_eff too just to see

# Use `coda` package
# Extract mcmc samples 
mcmc_sample <- as.mcmc(mp)
# Calculate effective sample size
...
p <- as_draws_df(mp) %>%
  mcmc_trace(pars = vars(b_Intercept:sigma),
    ...
    (p & scale_color_ipsum() &
    theme(legend.position = "right")) +
  plot_annotation(title = "Trace plots")

```
Both bulk_ESS and tail_ESS are a lot higher for parameter a than for b. The trace plot for a shows a good Markov chain of Gaussian shape&mdash; stationary and well-mixing. The trace plot for b wanders off to extreme values at some point, but if we consider the nature of Cauchy distribution&mdash; "Cauchy is a very thick-tailed distribution. At any moment in a Cauchy sampling process, it is possible to draw an extreme value that overwhelms all of the previous draws"&mdash; this behavior is expected of and normal for a Cauchy distribution. 
<br>
...
waic1 <- waic(m5.1)
waic2 <- waic(m5.2)
waic3 <- waic(m5.3) # warning: use `loo` instead

# Compare WAIC values
waic_values <- c(waic1$waic, waic2$waic, waic3$waic)
model_names <- c("m5.1", "m5.2", "m5.3")
comparison_waic <- data.frame(Model = model_names, WAIC = waic_values)
comparison_waic
...
# Compare loo values
loo_values <- c(loo1$loo, loo2$loo, loo3$loo)
model_names <- c("m5.1", "m5.2", "m5.3")
comparison_loo <- data.frame(Model = model_names, loo = loo_values)
comparison_loo

# Rank model based on waic and loo values 
# Ranking will be auto-done with `compare` function but all functions from 
# chapter 8 (1st edition) do not work anymore. So, I rank them manually here.
waic_values <- c(waic1$waic, waic3$waic, waic2$waic)
loo_values <- c(loo1$loo, loo3$loo, loo2$loo)
model <- c("m5.1", "m5.3", "m5.2")
comparison <- data.frame(Model = model, waic = waic_values, loo = loo_values)
comparison

```

Smaller waic values are better, with lower out of sample deviance, and likewise, lower loo values are indicative of a better fit. Among the three models, m5.1 seems to be the best with lowest waic and loo values, with m5.3 following closely behind (differences are rather small). 

<br>

> 8H3. Compare the two posterior distributions for m5.8s and m5.8s2. What has changed in the posterior distribution of both beta parameters? Can you explain the change induced by the change in prior?

```{r 8H3}

# Simulate height and leg lengths for 100 imagined individuals
N <- 100
height <- rnorm(N,10,2)
...
# ulam is working again, after troubleshooting based on this:
# https://discourse.mc-stan.org/t/problem-with-cmdstan-unpacking/29041/5
output <- capture.output(m5.8s <- ulam(
    alist(
        height ~ dnorm( mu , sigma ) ,
        mu <- a + bl*leg_left + br*leg_right ,
        a ~ dnorm( 10 , 100 ) ,
...
output <- capture.output(m5.8s2 <- ulam(
  ...
  data=d, chains=4, start=list(a=10,bl=0,br=1,sigma=1)))

precis(m5.8s2)
# Change in one prior cause changes in other prior due to correlations between 
# two beta parameters.

par(mfrow = c(1,2))
plot(m5.8s, main = "m5.8s")
plot(m5.8s2, main = "m5.8s2")

```

The precis results show the two parameters have negative correlation with each other&mdash; an increase in one causes a decrease in another. In the model with strictly positive prior, this negative correlation is preserved but the numbers are flipped&mdash; changes in direction of skewness. When br becomes strictly positive, bl skews more towards negative value. 
...
```{r 8H4, message=FALSE, warning=FALSE}

## I will use brms to calculate WAIC

# Original
output <- capture.output(b8h4a <-
          brm(data = d, family = gaussian,
...
# Strictly positive prior
output <- capture.output(b8h4 <-
        brm(data = d, family = gaussian,
            height ~ 1 + leg_left + leg_right,
            prior = c(set_prior("normal(10,100)", class = "Intercept"),
                      set_prior("normal(2,10)", class = "b", coef = "leg_left"),
                      set_prior("normal(2, 10)", class = "b", coef = "", lb = 0),
            # I set lower bound lb = 0 to make it half-normal-- not sure if it is right
...
b8h4a <- add_criterion(b8h4a, criterion = c("waic", "loo"))
b8h4 <- add_criterion(b8h4, criterion = c("waic", "loo"))

print(loo_compare(b8h4a, b8h4, criterion = "waic"), simplify = FALSE)
print(loo_compare(b8h4a, b8h4, criterion = "loo"), simplify = FALSE)


```

The WAIC values of the two models are very close&mdash; none is a better model than the other.
...
I cheated here. I just took the answer codes from the answer key and plug it in akjurz's codes. Anyway, the bars in the chart have uneven length now, showing the population size is not proportional to the island number anymore, but still to the number of stays. 
...
<span style="color:gray">***Given answer:***
...
I cheated again and copy from answer key because I couldn't solve this :)

```{r 8H6}

# To get a working Metropolis algorithm for this model, think of the different values 
# of p as the island indexes and the product of the likelihood and prior as the population 
# sizes. What is tricky here is that there an infinite number of islands: every continuous 
# value that p can take from zero to one. That’s hardly a problem, though. We just need 
# a different way of proposing moves, so that we can land on a range of islands.
num_samples <- 1e4
...
# This chain is fine, but it is more autocorrelated than a typical Stan chain. 
# That’s why it tends to wander around a bit. It is stationary, but it isn’t mixing 
# very well. This is common of such a simple Metropolis chain. But it is working.

# Now look at the posterior distribution implied by these samples. I’ll also compare it 
# to the analytically derived posterior for this model:
dens( p_samples , xlab="probability water" )
curve( dbeta(x,7,4) , add=TRUE , col="red" ) # Not bad curves.

#So what would you do if the model had more than one parameter? You just add another 
# proposal for each parameter, accepting or rejecting each proposal independent of the 
# others. Suppose for example we want to do a simple linear regression with a Metropolis 
# chain. Let’s simulate some simple Gaussian data and then compute the posterior distribution 
# of the mean and standard deviation using a custom Metropolis algorithm. 
